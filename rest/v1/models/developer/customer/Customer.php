...
class Customer
{
    ...
    // $columnAliasMap maps a frontend filter id to the real underlying
    // column when the SELECT aliases it (e.g. "is_active"), since aliases
    // aren't visible to a WHERE clause. This is shared by several read
    // methods that each alias different real columns to the same name
    // (customer_is_active, installment_payment_is_paid,
    // sales_order_is_active), so the map must be passed in per call site
    // rather than hardcoded here - only pass it from call sites where you've
    // confirmed which real column that call's SELECT aliases to that id.
    private function buildFilterColumns($allowedColumns, &$params, $columnAliasMap = [])
    {
        $filterColumn = [];
    ...
    public function readAll($allowedColumns)
    {
        ...
        $filterColumn = $this->buildFilterColumns($allowedColumns, $params, ["is_active" => "customer_is_active"]);
        try {
            $sql = "select *, ";
            $sql .= "customer_aid as id, ";
            $sql .= "customer_messenger as messenger, ";
